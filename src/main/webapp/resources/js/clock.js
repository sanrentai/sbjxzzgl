...
function getLastestJieQi() {
    var d = new Date();
    var jieqi = "";
    var i = 0;
    for ( max = 30; i < max; i++) {
        jieqi = getjq(d.getFullYear(), d.getMonth()+1, d.getDate());
        if (jieqi !== "") {
            break;
        }
        d = +d + 1000*60*60*24;
    ...
    var days = new Array("今天是","一天后","两天后","三天后","四天后","五天后","六天后","七天后","八天后","九天后","十天后","十一天后","十二天后","十三天后","十四天后","十五天后","十六天后");
//    alert(days[i] + " " + jieqi);
    return days[i] + jieqi;
}
////节气函数结束////
